...
# 添加目录监控。
auditctl -a always,exit -S open mkdir create link syslink -F key=test_file_create -w '/home'
auditctl -a always,exit -S open mkdir create link syslink -F key=test_file_create -w '/data'

# 删除一个已添加的审计监控
auditctl -d always,exit -S open mkdir create link syslink -F key=test_file_create -w '/data'
